...
import { replay, type CaseState, type CaseStatus } from './projection';
...
export function buildQueue(): QueueView {
  const items = allCaseStates().map(toQueueItem);
...
function toApprovalView(state: CaseState): ApprovalView | null {
  const approval = state.approval;
  if (!approval) return null;

  const sealValid = verifySeal(
    {
      caseId: state.caseId,
